# -*- coding: utf-8 -*-
"""
Created on Thu Oct 22 17:07:17 2020
...
class InputKndPad():
    ...
    @classmethod
    def _getKndPadCompRxnDistInfo(cls, dirSourceData = "../sourcedata/",
                                       resource_xlsxFile = "KndPad_Ori_Cleaned_Half.xlsx",
                                       metacyc_xref_kndpad_jsonFile = "MetaCyc_Xref_KndPad.json",
                                       chebi_rhea_xref_kndpad_jsonFile = "ChEBI_Rhea_Xref_KndPad.json",
                                       kegg_xref_kndpad_jsonFile = "KEGG_Xref_KndPad.json"
                                       ):
        """
        ...
        #
        chebi_rhea_xref_kndpad_dict = dict()
        chebi_rhea_xref_kndpad_dict["Compound"] = dict()
        chebi_rhea_xref_kndpad_dict["Reaction"] = dict()
        #
        kegg_xref_kndpad_dict = dict()
        kegg_xref_kndpad_dict["Compound"] = dict()
        kegg_xref_kndpad_dict["Reaction"] = dict()
        #
        compInPsRxnSet = set()
        rxnInPsSet = set()
        ...
        for rxnInfo in rxnInfoList[1:]:  # Exclude title
            ...
            rxnCompList = re.findall("Met\d{6}-[mck]", rxnEquation)
            ...
            #
            rhea = json.loads(rxnInfo[16])                              # Rhea
            if len(rhea) == 1:
                try:
                    # 1st,MetaCyc; 2nd,Rhea; 3rd,KEGG;
                    chebi_rhea_xref_kndpad_dict["Reaction"][rhea[0]]
                except KeyError:
                    chebi_rhea_xref_kndpad_dict["Reaction"][rhea[0]] = rxnId
            #
            kegg = json.loads(rxnInfo[17])                              # KEGG
            if len(kegg) == 1:
                try:
                    # 1st,MetaCyc; 2nd,Rhea; 3rd,KEGG;
                    kegg_xref_kndpad_dict["Reaction"][kegg[0]]
                except KeyError:
                    kegg_xref_kndpad_dict["Reaction"][kegg[0]] = rxnId
            #
        ...
        for idx, compInfo in enumerate(compInfoList[1:]):  # Exclude title
            ...
            if compId in compInPsRxnSet:
                ...
                if compId.endswith('-m'):
                    source = source.replace("MetaCyc:", "")
                    source = source.replace("%2b", "+")
                    metacyc_xref_kndpad_dict["Compound"][source] = compId
                #
                chebi = json.loads(compInfo[10])                        # ChEBI
                if len(chebi) == 1:
                    try:
                        # 1st,MetaCyc; 2nd,ChEBI; 3rd,KEGG;
                        chebi_rhea_xref_kndpad_dict["Compound"][chebi[0]]
                    except KeyError:
                        chebi_rhea_xref_kndpad_dict["Compound"][chebi[0]] = compId
                #
                kegg = json.loads(compInfo[11])                         # KEGG
                if len(kegg) == 1:
                    try:
                        # 1st,MetaCyc; 2nd,ChEBI; 3rd,KEGG;
                        kegg_xref_kndpad_dict["Compound"][kegg[0]]
                    except KeyError:
                        kegg_xref_kndpad_dict["Compound"][kegg[0]] = compId
                #
        ...
        #
        chebi_rhea_xref_kndpad_jsonFile = path.join("datas", chebi_rhea_xref_kndpad_jsonFile)
        with open(chebi_rhea_xref_kndpad_jsonFile, "w") as file_obj:
            file_obj.write(json.dumps(chebi_rhea_xref_kndpad_dict, indent=2))
        #
        kegg_xref_kndpad_jsonFile = path.join('datas', kegg_xref_kndpad_jsonFile)
        with open(kegg_xref_kndpad_jsonFile, "w") as file_obj:
            file_obj.write(json.dumps(kegg_xref_kndpad_dict, indent=2))
        #
        return [compDistInfoList, rxnDistInfoList]


    @classmethod
    def _getKndPadRxnInfo(cls, dirSourceData = "../sourcedata/",
                               resource_xlsxFile = "KndPad_Ori_Cleaned_Half.xlsx",
                               ):
        """
            Get info of every reaction's coef/coefSum/compID/canoSmiles/nonIsoSmiles and
        isoReaProReactionIDs from the cleaned KndPad reaction excle(InfoEx).
    ...
    @classmethod
    def _getKndPadGeCosInfo(cls, dirDatas ="datas/",
                                 general_cofactors_xlsxFile = "KndPad_General_Cofactors.xlsx",
                                 ):
        """ """
    ...
    @classmethod
    def _getKndPadPsRxnDirInfo(cls, ):
        """ """
        rxnDirInfoDict = dict()
    ...
    @classmethod
    def _getKndPadPsRxnCompDrPairInfo(cls, dirSourceData = "../sourcedata/",
                                           resource_xlsxFile = "KndPad_Ori_Cleaned_Half.xlsx",
                                           dirDatas = "datas/",
                                           general_cofactors_xlsxFile = "KndPad_General_Cofactors.xlsx",
        ...
        """
        #
        with open(path.join("temp", "KndPad_Info.data"), "rb") as file_obj:
            [rxnCoefCompSmiDict, rxnIsoReaProIdSet] = pickle.load(file_obj)
        #
        """
        [rxnCoefCompSmiDict, rxnIsoReaProIdSet] = cls._getKndPadRxnInfo(dirSourceData = dirSourceData,
                                                                        resource_xlsxFile = resource_xlsxFile,
                                                                        )
        """
        #
        sdqFamilyList = cls._getKndPadGeCosInfo(dirDatas = dirDatas,
                                                general_cofactors_xlsxFile = general_cofactors_xlsxFile,
                                                )
        #
        exSubStrTupleList = FingerPrint.getExSubStrInfo(dataBase = "KndPad")
        #
    ...
    @classmethod
    def _fuseKndPadPsRxnCompPairInfo(cls, rxnDistInfoList, rxnDirInfoDict, rxnDrCompPairsInfoDict,
                                          diffThresholdLevel = 10,
                                          ):
        """
        """
        ...
        rxnDirCompPairDict = Reconcile.reconcileKndPad()
        #
    ...
    @classmethod
    def _saveKndPadPsRxnCompPairInfo(cls, compFuseInfoList, rxnFuseInfoList,
                                          dirSourceData = "../sourcedata/",
                                          xlsxFile = "KndPad_Pathway_Search_Ori.xlsx",
                                          diffThresholdLevel = 10,
                                          ):
        """ """
    ...
    def _generateInputPrompt(cls, compFuseInfoList, rxnFuseInfoList,
                                  inputprompt_jsonFile = "KndPad_InputPrompt.json",
                                  ):
        """
        """
        #
        ...
        #
        inputprompt_kndpad_dict = dict()
        inputprompt_kndpad_dict["Compound"] = comp_inputprompt_list
        inputprompt_kndpad_dict["Reaction"] = rxn_inputprompt_list
        #
        with open(inputprompt_jsonFile, "w") as file_obj:
            file_obj.write(json.dumps(inputprompt_kndpad_dict, indent=2))
        #
    ...
    def loadRxnCompPair(cls, dirSourceData = "../sourcedata/",
        ...
        """
            Load compound pairs of every reaction from KndPad.
        """
        #
        data_file = "KndPad_Dmn.data"
        xlsxFile = path.join(dirSourceData, xlsxFile)
    ...
    def _getOrgCompRxnId(cls, org_model_location = "../sourcedata/models/",
                              ...
                              xref_chebirhea2kndpad_json = "datas/ChEBI_Rhea_Xref_KndPad.json",
                              xref_kegg2kndpad_json = "datas/KEGG_Xref_KndPad.json",
                              xref_metacyc2bigg_json = "datas/KndPad_Xref_BiGG.json",
                              ):
        """
            Get the information of the selected organism from bigg.
            And we try to map compounds and reactions in KndPad to the metabolites and metabolic reactions in cytosol
        of this organism.
        """
        #
        ...
        #
        with open(xref_chebirhea2kndpad_json, "r") as file_obj:
            chebi_rhea_xref_kndpad_dict = json.load(file_obj)
        #
        with open(xref_kegg2kndpad_json, "r") as file_obj:
            kegg_xref_kndpad_dict = json.load(file_obj)
        #
        # "bsu": "bsu_iYO844.json",
        ...
        for met_info_dict in metabolite_list:
            ...
            #
            try:
                biocyc_comp_list = met_annotation_dict['biocyc']        # BioCyc(MetaCyc)
                for biocyc_comp_id in biocyc_comp_list:
                    # biocyc --> metacyc --> kndpad
                    biocyc_comp_id = biocyc_comp_id.replace("META:", "")
                    try:
                        kndpad_comp_id = metacyc_xref_kndpad_dict["Compound"][biocyc_comp_id]
                    except KeyError:
                        continue
                    try:
            ...
            #
            try:
                chebi_comp_list = met_annotation_dict['chebi']          # ChEBI
                for chebi_comp_id in chebi_comp_list:
                    chebi_comp_id = chebi_comp_id.replace("CHEBI:", "")
                    try:
                        # chebi --> kndpad
                        kndpad_comp_id = chebi_rhea_xref_kndpad_dict["Compound"][chebi_comp_id]
                    except KeyError:
                        continue
                    try:
                        if comp_metacyc2model_dict[kndpad_comp_id].endswith('_c'):  # cytosol
            ...
            #
            try:
                kegg_comp_list = met_annotation_dict['kegg.compound']   # KEGG
                for kegg_comp_id in kegg_comp_list:
                    # kegg_comp_id = kegg_comp_id.replace("", "")
                    try:
                        # kegg --> kndpad
                        kndpad_comp_id = kegg_xref_kndpad_dict["Compound"][kegg_comp_id]
                    except KeyError:
                        continue
                    try:
                        if comp_metacyc2model_dict[kndpad_comp_id].endswith('_c'):  # cytosol
        ...
        for rxn_info_dict in reaction_list:
            ...
            #
            try:
                biocyc_reac_list = rxn_annotation_dict['biocyc']        # BioCyc(MetaCyc)
                for biocyc_reac_id in biocyc_reac_list:
                    # biocyc --> metacyc --> kndpad
                    biocyc_reac_id = biocyc_reac_id.replace("META:", "")
                    try:
                        kndpad_reac_id = metacyc_xref_kndpad_dict["Reaction"][biocyc_reac_id]
            ...
            #
            try:
                rhea_reac_list = rxn_annotation_dict['rhea']            # Rhea
                for rhea_reac_id in rhea_reac_list:
                    # rhea --> kndpad
                    # rhea_reac_id = rhea_reac_id.replace("RHEA:", "")
                    try:
                        kndpad_reac_id = chebi_rhea_xref_kndpad_dict["Reaction"][rhea_reac_id]
                        reac_metacyc2model_dict[kndpad_reac_id] = rxn_id
                    except KeyError:
                        pass
            except KeyError:
            ...
            #
            try:
                kegg_reac_list = rxn_annotation_dict['kegg.reaction']   # KEGG
                for kegg_reac_id in kegg_reac_list:
                    # kegg --> kndpad
                    # kegg_reac_id = kegg_reac_id.replace("", "")
                    try:
                        kndpad_reac_id = kegg_xref_kndpad_dict["Reaction"][kegg_reac_id]
                        reac_metacyc2model_dict[kndpad_reac_id] = rxn_id
                    except KeyError:
                        pass
            except KeyError:
